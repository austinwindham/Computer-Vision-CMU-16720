...
for itr in range(max_iters):
    ...
    for xb,yb in batches:
        ...
        ##### your code here #####
        ##########################
    # initialized avg_acc but it iis cumulative
...
h1 = forward(batch_x, params, 'layer1')
probs = forward(h1, params,'output', softmax)
error_f = probs-batch_y
error_b = backwards(error_f, params,'output', linear_deriv)
backwards(error_b, params, 'layer1',sigmoid_deriv)
...
for k,v in params.items():
    ...
    if 'W' in k:
        for n in range(v.shape[0]):
            for m in range(v.shape[1]):
                ...
                params_r[k] = v+ eps*d
                h1 = forward(batch_x, params_r, 'layer1')
                probs = forward(h1, params_r, 'output',softmax)
                loss_r, acc = compute_loss_and_acc(batch_y, probs)

                #shift left
                params_l = copy.deepcopy(params)
                params_l[k] = v- eps*d
                h1 = forward(batch_x, params_l, 'layer1')
                probs = forward(h1, params_l, 'output',softmax)
                loss_l, acc = compute_loss_and_acc(batch_y, probs)

                params['grad_'+k][n, m] = (loss_r-loss_l)/(2*eps)
    ...
    if 'b' in k:
        for i in range(v.shape[0]):
            ...
            params['grad_'+k][i] = (loss_r-loss_l)/(2*eps)
