...
np.save('girlseqrects-wcrt.npy', car_rects)


# plotter same as car, just upload girl  files
seq = np.load("../data/girlseq.npy")
car = np.load('girlseqrects.npy')
car_corrected = np.load('girlseqrects-wcrt.npy')
...
for i in range(len(car)):
    if i in [0, 19, 39, 59, 79]:
        ...
        ax.add_patch(patch)
        ax.add_patch(patch2)
        fig.savefig('girlcorrected'+str(i+1)+'.png', bbox_inches='tight')